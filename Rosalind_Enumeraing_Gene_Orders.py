...
def permutations(n):
    ...
    list_of_integers = list((range(1, n + 1)))
    permutations = list(itertools.permutations(list_of_integers))
    print(len(permutations))
    for subLst in permutations:
        print(" ".join([str(ele) for ele in subLst]))

def main():
    with open('./txt_files/rosalind_perm.txt') as infile:
